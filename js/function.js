...
function toggle_SideBar() {
  ...
  if ($('div.sidebar').is(':hidden')) {
    $('div.sidebar').show(0);
    $("div.Map-container").animate({
      width: "75%",
    }, 1500, function () {
      $('.fa-arrow-circle-right').hide(0);
    });
...
function show_hide_Setting() {
  var data_height = $('div.data-list').height();
  // 6rem = 16px*6
  if ($('div.setting').height() == 96) {
    $('div.setting').animate({
      height: "0%"
    }, 500, function () {
      // 因為這邊的高度會調整，所以位於該div下的 data-list的高度也要調整
      $('div.data-list').height(data_height + 96);
      $('div.setting').hide();
    });
  } else {
    $('div.setting').show();
...
//   if (this.checked) {
//     $('input#select_MRT').prop("checked", true);
//     $('input#select_TRA').prop("checked", true);
//   } else {
//     $('input#select_MRT').prop("checked", false);
//     $('input#select_TRA').prop("checked", false);
//   }
